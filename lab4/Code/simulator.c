...
void add_memory_var_desc(pOperand op){
	struct VarDesc* new_var = (struct VarDesc*)malloc(sizeof(struct VarDesc));
	new_var->op=op;
	new_var->next=NULL;
...
int ensure(pOperand op){//保证op在寄存器中，返回寄存器编号
	struct VarDesc* p = var_head;
	while(p!=NULL)
	{
...
void trans_one_line(pInstruction curr,FILE *fp){
	switch (curr->kind) {
		...
		case 3:{
			...
			else
			{
				...
				{	
					...
					{
						int result_reg_num = ensure(tmp->u.singleop.op);
		...
		case ASSIGN:{
			int result_reg_num = ensure(curr->u.assign.result);
			if(curr->u.assign.op1->kind == CONSTANT){
				fprintf(fp,"  li %s, %d\n",_reg[result_reg_num].name,curr->u.assign.op1->u.value);
			}
			else{
				int op_reg_num = ensure(curr->u.assign.op1);
		...
		case ADD:{
			...
			int result_reg_num = ensure(result);
		...
		case ADDR:{
			int result_reg_num = ensure(curr->u.assign.result);
			int op1_reg_num = ensure(curr->u.assign.op1);
		...
		case IF_GOTO:{
			int x_num = ensure(curr->u.if_goto.x);
			int y_num = ensure(curr->u.if_goto.y);
			fprintf(fp,"  ");
		...
		case RETURN:{
			...
			{
				int reg_num = ensure(curr->u.singleop.op);
		...
		case CALL:{
			int result_reg_num = ensure(curr->u.singleop.op);
			...
			while(tmp != NULL && tmp->kind == ARG)
			{
				...
				{
					int reg_num = ensure(tmp->u.singleop.op);
		...
		case WRITE:{
			if(in_func==0)
			{
				int result_num = ensure(curr->u.singleop.op);
