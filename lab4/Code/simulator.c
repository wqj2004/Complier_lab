...
void add_var_desc(int reg, pOperand op, FILE *fp)
{
	...
	if (op->kind == VARIABLE)
	{
		pobj obj = searchtab(table, op->u.name);
		if (obj != NULL && obj->type->kind == ARRAY)
		{
			fprintf(fp, "  la %s, %s\n", _reg[reg].name, op->u.name);
		}
	}
}
...
int allocate(pOperand op, FILE *fp)
{
	...
	while (p != NULL)
	{
		if (p->op->kind == CONSTANT && p->reg_num != last_changed_reg)
		{ // 如果是常量且不是最后改变的寄存器
...
int ensure(pOperand op, FILE *fp)
{ // 保证op在寄存器中，返回寄存器编号
	struct VarDesc *p = var_head;
	while (p != NULL)
	{
...
void trans_one_line(pInstruction curr, FILE *fp)
{
	...
	case MUL:
	{
		...
		{ // 其中一个是常量也必须加载到寄存器中，否则不满足MIPS32语法规范
			int op1_reg_num = ensure(op1, fp);
			int op2_reg_num = ensure(op2, fp);
			if (op1->kind == CONSTANT)
			{
