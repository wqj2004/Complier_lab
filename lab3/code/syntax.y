...
FunDec : ID LP VarList RP   { $$=create_node("FunDec",type_nter, @$.first_line,4,$1,$2,$3,$4); } //有参数的函数声明
    ...
    | ID LP error RP        { yyerror("syntax FunDec error"); yyerrok;}
    |error LP VarList RP    { yyerror("syntax FunDec error"); yyerrok;}
    ;
...
Stmt : Exp SEMI                                 { $$=create_node("Stmt",type_nter, @$.first_line,2,$1,$2); }//表达式
    ...
    | Exp error SEMI                            { yyerror("syntax Stmt error"); yyerrok;}
    | RETURN Exp error                           { yyerror("syntax Stmt error"); yyerrok;}
    | RETURN error SEMI                          { yyerror("syntax Stmt error"); yyerrok;}
    ;
...
Def : Specifier DecList SEMI    { $$=create_node("Def",type_nter, @$.first_line,3,$1,$2,$3); } //单个局部定义,由标识符+声明列表+;组成
    |Specifier error SEMI       { yyerror("syntax Def error"); yyerrok;}
    |Specifier DecList error    { yyerror("syntax Def error"); yyerrok;}
    ;
...
Exp : Exp ASSIGNOP Exp      { $$=create_node("Exp",type_nter, @$.first_line,3,$1,$2,$3); }
    ...
    | Exp ASSIGNOP error     { yyerror("syntax Exp error"); yyerrok;};
    | Exp AND error          { yyerror("syntax Exp error"); yyerrok;};
    | Exp OR error           { yyerror("syntax Exp error"); yyerrok;}
    | Exp RELOP error        { yyerror("syntax Exp error"); yyerrok;}
    | Exp PLUS error         { yyerror("syntax Exp error"); yyerrok;}
    | Exp MINUS error        { yyerror("syntax Exp error"); yyerrok;}
    | Exp STAR error         { yyerror("syntax Exp error"); yyerrok;}
    | Exp DIV error          { yyerror("syntax Exp error"); yyerrok;}
    | LP error RP            { yyerror("syntax Exp error"); yyerrok;}
    | MINUS error            { yyerror("syntax Exp error"); yyerrok;}
    | NOT error              { yyerror("syntax Exp error"); yyerrok;}
    | ID LP error RP         { yyerror("syntax Exp error"); yyerrok;}
    | Exp LB error RB        { yyerror("syntax Exp error"); yyerrok;}
    ;
